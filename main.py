...
def xtime_tensor(a: Tensor) -> Tensor:
    # Get high bit mask (equivalent to a & 0x80)
    high_bit_mask = a.bitwise_and(0x80)
    
    # Compute left shift for all values
    shifted = a.lshift(1)
    
    # Create the conditional xor with 0x1B 
    condition = high_bit_mask != 0
    result = condition.where(
        shifted.xor(0x1B),  # when high bit is set
        shifted             # when high bit is not set
    )
    
    # Mask to keep only bottom 8 bits
